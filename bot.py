...
from database.db import get_theme, format_price
...
# --- Web Server Endpoints for Telegram Mini App ---
async def health_check(request):
    return web.Response(text="✅ Telegram Service Bot & Mini App are running 24/7!", content_type="text/plain")
...
async def api_get_theme(request):
...
async def api_create_order(request):
    ...
    try:
        ...
        # Record user if real user_id provided
        if user_id > 0:
        ...
        # Notify Client on Telegram (if user_id valid)
        if bot_instance and user_id > 0:
            try:
                client_msg = (
                    f"🎉 **Order Placed Successfully via Mini App! (Order #{order_id})**\n\n"
                    f"🏷️ **Service:** {service['name']}\n"
                    f"💵 **Price:** `{price_display}`\n"
                    f"⏱️ **Duration:** `{service['duration']}`\n\n"
                    f"📋 **Requirements:**\n_{requirements}_\n\n"
                    "Our team has received your order and will begin work right away!"
                )
        ...
        if bot_instance:
            admin_alert_text = (
                f"🚨 **NEW MINI-APP ORDER! (Order #{order_id})**\n\n"
...
async def start_web_server():
    """Starts the lightweight web server serving the Mini App & Health Check"""
    ...
    app.router.add_get("/api/theme", api_get_theme)
    ...
    await site.start()
    logger.info(f"🌐 Health & Mini App server running on port {port}")
...
async def main():
    ...
    # Start health & mini-app web server
    try:
